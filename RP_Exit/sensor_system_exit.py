...
class SensorSystem:
    ...
    def on_connect(self, client, userdata, flags, reason_code, properties):
        ...
        if self.gate_type == "entry":
            topic_indxs = entry_topics_subscribe
        elif self.gate_type == "exit":
            topic_indxs = exit_topics_subscribe
        topics_list = list(topics.keys())

        for i in topic_indxs:
            client.subscribe(topics[topics_list[i]], qos=self.default_qos)
    ...
    def on_message(self, client, userdata, msg):
        ...
        if topic == topics["exit_ultr_sensor_req"]:
            ...
            self.publish(distance, topics["exit_ultr_sensor_resp"])
    ...
    def get_exit_ultra_distance(self):
        ...
        while not self.reset.is_set() and not self.distance_event.is_set():
            time.sleep(0.01)
        time.sleep(0.01)
